...
Matrix::Matrix (const Matrix &that) {
    ...
    for (size_t i = 0; i < rows; ++i) {
        for (size_t j = 0; j < columns; ++j) {
            ...
            matrix[index(i, j)] = that(i, j);
        }
    }
}

// move constructor
Matrix::Matrix(Matrix&& that) : rows(0), columns(0), matrix(nullptr) {
    swap(that);
}
...
void Matrix::operator+= (const Matrix &addend) {
    assert((rows == addend.rows) && (columns == addend.columns));
    for (size_t i = 0; i < rows; ++i) {
        for (size_t j = 0; j < columns; ++j) {
...
void Matrix::operator-= (const Matrix &subtrahend) {
    assert((rows == subtrahend.rows) && (columns == subtrahend.columns));
    for (size_t i = 0; i < rows; ++i) {
        for (size_t j = 0; j < columns; ++j) {
